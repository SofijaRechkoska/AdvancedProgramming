...
class Equation <IN,OUT>{
    private Supplier<IN> input;
    private Function<IN,OUT> output;

    Equation(Supplier<IN> input, Function<IN, OUT> output) {
        this.input = input;
        this.output = output;
    }

    public Optional<OUT> calculate(){
        return Optional.of(output.apply(input.get()));
    }
}
...
class EquationProcessor{
    public static <IN,OUT>void process (List<IN> inputs,List<Equation<IN,OUT>> equationList){
        ...
        for (Equation<IN,OUT> equation:equationList){
...
public class EquationTester {
    public static void main(String[] args) {
        ...
        if (testCase == 1) { // Testing with Integer, Integer
            ...
            equations2.add(new Equation<>(
                    ()->inputs.get(0),
                    s-> inputs.stream().filter(
                            x->x.calculateLine() > inputs.get(0).calculateLine()
