package LabExercises.Lab3;//package LabExercises.Lab3;
...
class EmptyResultException extends Exception {
    EmptyResultException(String msg) {
        super(msg);
    }
}
    ...
    public static void main(String[] args) {
        ...
        if (testCase == 1) { // students
            ...
            if (studentScenario == 1) {
                ...
                try {
                    List<Student> studentList = FilterAndSort.execute(students,
                            student -> student.labAssistantPoints() >= 8.0 && student.getYear() >= 3
                    );
                ...
                try {
                    List<Student> studentList = FilterAndSort.execute(
                            students,
                            student -> student.grades.size() > 0.9 * student.totalCourses() && student.average() >= 9.0
                    );
                    studentList.stream().forEach(System.out::println);
            ...
            try {
                List<Integer> integerList = FilterAndSort.execute(
                        integers,
                        integer -> integer % 15 == 0
                );
